...
/* Define New CAN Command Strings */
#define CMD_SET_TAU                     "SET_TAU"
#define CMD_SET_KAW                     "SET_KAW"
#define CMD_SET_DISTURBANCE_THRESHOLD   "SET_DISTURBANCE_THRESHOLD"
...
static bool CAN_ParseMessage(const char *message) {
    ...
    /* Define maximum expected command length */
    ...
    if (sscanf(message, "%63[^=]=%63s", command, value_str) != 2) {
        return false;
    }

    /* Process commands using helper functions for robust parsing */
    if (strcmp(command, "SET_MAX_RPM") == 0) { 				// SET_MAX_RPM
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value > 10000) { // Define MAX_RPM_LIMIT
            return false;
        }
        config.maxRPM = (int)value;
    }
    else if (strcmp(command, "SET_ENCODER_TYPE") == 0) {	// SET_ENCODER_TYPE
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value >= ENCODER_TYPE_COUNT) { // Define ENCODER_TYPE_COUNT
            return false;
        }
        config.encoderType = (EncoderType_t)value;
    }
    else if (strcmp(command, "SET_ENCODER_COMM") == 0) { 	// SET_ENCODER_COMM
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value >= ENCODER_COMM_COUNT) { // Define ENCODER_COMM_COUNT
            return false;
        }
        config.encoderComm = (EncoderComm_t)value;
    }
    else if (strcmp(command, "SET_TARGET_ANGLE") == 0) {	// SET_TARGET_ANGLE
        double value;
        if (!ParseDouble(value_str, &value, 0.0, 360.0)) { // Define MIN_ANGLE and MAX_ANGLE
            return false;
        }
        config.targetAngle = value;
    }
    else if (strcmp(command, "SET_SPEED") == 0){			// SET_SPEED
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value > 5000) { // Define MAX_SPEED_LIMIT
            return false;
        }
        config.maxSpeed = (int)value;
    }
    else if (strcmp(command, "POWER_MODE") == 0){			// POWER_MODE
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value >= POWER_MODE_COUNT) { // Define POWER_MODE_COUNT
            return false;
        }
        config.powerMode = (PowerMode_t)value;
    }
    else if (strcmp(command, "SET_ACC") == 0){				// SET_ACC
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value > 1000) { // Define MAX_ACC_LIMIT
            return false;
        }
        config.maxAcceleration = (int)value;
    }
    else if (strcmp(command, "SET_JERK") == 0){				// SET_JERK
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value > 1000) { // Define MAX_JERK_LIMIT
            return false;
        }
        config.maxJerk = (int)value;
    }
    else if (strcmp(command, "SET_LIMIT_UP") == 0 ){		// SET_LIMIT_UP
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < -1000 || value > 1000) { // Define MIN_POSITION_LIMIT and MAX_POSITION_LIMIT
            return false;
        }
        config.positionLimitUpper = (int)value;
    }
    else if (strcmp(command, "SET_LIMIT_DOWN") == 0 ){		// SET_LIMIT_DOWN
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < -1000 || value > 1000) { // Define limits
            return false;
        }
        config.positionLimitLower = (int)value;
    }
    else if (strcmp(command, "SET_ERROR_LIM") == 0 ){		// SET_ERROR_LIM
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
        if (endptr == value_str || *endptr != '\0' || errno == ERANGE || value < 0 || value > 100) { // Define MAX_ERROR_THRESHOLD
            return false;
        }
        config.maxErrorThreshold = (int)value;
    }
    else if (strcmp(command, "SET_FEEDBACK") == 0 ){		// SET_FEEDBACK
        char *endptr;
        errno = 0;
        long value = strtol(value_str, &endptr, 10);
    ...
    else if (strcmp(command, "SET_PID_KP") == 0 ){ 			// SET_PID_KP
        double value;
        if (!ParseDouble(value_str, &value, 0.0, 100.0)) { // Define MIN_PID_KP and MAX_PID_KP
            return false;
        }
        config.kp = value;
    }
    else if (strcmp(command, "SET_PID_KI") == 0 ){ 			// SET_PID_KI
        double value;
        if (!ParseDouble(value_str, &value, 0.0, 100.0)) { // Define MIN_PID_KI and MAX_PID_KI
            return false;
        }
        config.ki = value;
    }
    else if (strcmp(command, "SET_PID_KD") == 0 ){ 			// SET_PID_KD
        double value;
        if (!ParseDouble(value_str, &value, 0.0, 100.0)) { // Define MIN_PID_KD and MAX_PID_KD
            return false;
        }
        config.kd = value;
    }
    /* New Commands for Enhanced PID */
    else if (strcmp(command, CMD_SET_TAU) == 0) {			// SET_TAU
        double value;
    ...
    else if (strcmp(command, CMD_SET_KAW) == 0) {			// SET_KAW
        double value;
    ...
    else if (strcmp(command, CMD_SET_DISTURBANCE_THRESHOLD) == 0) { // SET_DISTURBANCE_THRESHOLD
        double value;
